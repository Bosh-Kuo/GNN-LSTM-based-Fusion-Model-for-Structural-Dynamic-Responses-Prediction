...
class GAT_Encoder(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr ,batch):
        ...
        x = x.relu()
        x, (edge_index_returned, attention_weights) = self.conv3(x, edge_index, edge_attr, return_attention_weights = True)
        x = self.dropout(x)
...
class GAT_LSTM(nn.Module):
    def __init__(self, GAT_Encoder, LSTM, device):
    ...
    def forward(self, Data):
        ...
        # GNN
        # gnn_embedding = [batch_size, gnn_embed_dim]
        gnn_embedding, edge_index_returned, attention_weights = self.GAT_Encoder(x, edge_index, edge_attr, batch)

        # LSTM
        # output = [batch, original_trg_seq_len, max_story]
        output = self.LSTM(ground_motion, gnn_embedding, seq_length_list)

        return output, edge_index_returned, attention_weights


class GAT_Only(nn.Module):
    def __init__(self, GAT_Encoder, LSTM, device):
        super(GAT_Only, self).__init__()
        self.GAT_Encoder = GAT_Encoder
        self.LSTM = LSTM
        self.device = device
