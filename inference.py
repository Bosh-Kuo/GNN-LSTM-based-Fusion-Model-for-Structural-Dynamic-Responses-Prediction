...
# * create folder
eval_fig_dir = os.path.join(args.output_dir, "eval_figure")
...
with torch.no_grad():
    for i, Data in enumerate(tqdm(eval_loader)):
        ...
        ## Plot Attention
        x_coord = (
            Data.x[:, 3].numpy() * normalized_item_dict["x"]["XYZ_grid_index"].item()
        )
        y_coord = (
            Data.x[:, 5].numpy() * normalized_item_dict["x"]["XYZ_grid_index"].item()
        )
        z_coord = (
            Data.x[:, 4].numpy() * normalized_item_dict["x"]["XYZ_grid_index"].item()
        )
